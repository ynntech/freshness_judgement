...
var pace: TimeInterval = 0.5 // in seconds, classification will not repeat faster than this value
...
class ViewController: UIViewController,UIImagePickerControllerDelegate,UINavigationControllerDelegate {
    ...
    @IBAction func launchCamera(_ sender: UIBarButtonItem) {
        ...
        let camera = UIImagePickerControllerSourceType.photoLibrary //ここcameraにするとcameraが立ち上がる。
 ...
 */
  ...
  lazy var classificationRequest: [VNRequest] = {
    do {
      ...
      // ここで自分のモデルにあったように書き換え．
      //let model = try VNCoreMLModel(for: Fruit().model)
    let model = try VNCoreMLModel(for: spajam().model)
        //ここで，VisionフレームワークからCore MLへのリクエスト
        // リクエスト（VNCoreMLRequest）の生成
  ...
  func handleClassification(request: VNRequest, error: Error?) {
    ...
    //?でオプショナル型にしてobservationsに代入して，nilがなければ抜け出し，nilならfaralErrorにする．
    ...
    //ターミナルに出力． (includes basic filtering)
    print("\(best.identifier): \(best.confidence)")
    
    //best.identifierがラベル, best.confidenceが何%か．
    ...
    
    {
      ...
      DispatchQueue.main.async {
        ...
        //GUIに出力　ここで文字の色とか整形．
        self.bubbleLayer.string = /*"これは"+*/best.identifier.trimmingCharacters(in: CharacterSet.whitespacesAndNewlines)/*+"だよ"*/
      }
